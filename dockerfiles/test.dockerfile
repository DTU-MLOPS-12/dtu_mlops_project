...
# Build with secret
# docker build --secret id=GCP_SA_KEY_DVC -f dockerfiles/test.dockerfile -t test:latest .

# Debug
# docker run -it --entrypoint bash test:latest
...
#COPY *.dvc .dvc/
#COPY data/processed/timm-imagenet-1k-wds-subset.dvc data/processed/timm-imagenet-1k-wds-subset.dvc
